...
def kr_regression(y_train, y_vali, y_trains, y_valis, x_train, x_vali, kfold, fileName, modelName):
    """
    performs kernel regression with an RBF-kernel and saves the scores and the trained model in a file. The training is
    ...
    lr = KernelRidge(alpha=0.9, kernel='rbf')
    ...
    data_functions.getScores(fileName, y_trains, y_valis, pred_train, pred_vali, modelName, len(lr.get_params()), kfold,
...
def main(inFilePairStat, sos1, featureDecisionFile, fileName, modelName):
    ...
    x_data_grav, y_data_grav = data_functions.getGravityData(inFilePairStat, sos1, decide,
                                                             featureDecisionFile, True)
    x_data_grav_score, y_data_grav_score = \
        data_functions.getGravityDataScores(inFilePairStat, sos1, decide, featureDecisionFile)
    kr_regression(y_data_grav, [], y_data_grav_score, [], x_data_grav, [], 0, fileName, modelName)
